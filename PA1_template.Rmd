...
## Reproducible reserach Assignment one
...
### 3. Ignoring the missing values in the data set, and checking the average daily activity pattern
...
```
Based on the imputed data the mean of the total number of steps taken per day is `r round(stn_mean)`    and the median of the total number os steps taken per day is `r round(stn_median)`. There is hardly any difference from the mean and median of the data set without the missing values.
But comparing figure 1 with figure 3 we can observe that the frequency of cetaon intervals have increased, because we have substituted the missing values with the mean for that interval over all the days. There is hardly any change in the shape of the histogram for the two data sets.
...
WE can observe that there is a distinct difference between the pattern for weekdays in comparison to that for weekends. For weekdays we can see that activity peaks around midday, wheras during weekends we see spikes of activities at various times of the day, not just one during midday.